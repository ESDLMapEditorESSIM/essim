...
/**
 * Contains converter methods between the dido specification (ecore model) and the runtime
 * 
 * @author werkmane
 *
 */
public class Converter {
	 ...
	 * It uses {@link DurationUnitEnum#SECOND} as duration Unit as java.time.Duration is defined in seconds.
	 * 
	 * @param duration
	 *            in java.time version
	...
	public static esdl.Duration toESDLDuration(EssimDuration duration) {
		esdl.Duration esdlDuration = EsdlFactory.eINSTANCE.createDuration();
		// TODO: BAD HACK! test if this works: parse based on removing the last 'S' from one enum to the other
		esdlDuration.setDurationUnit(DurationUnitEnum.valueOf(duration.getUnit()
				.name()
				.substring(0, duration.getUnit()
						.name()
						.length() - 1)));
	...
	public static EssimDuration toEssimDuration(esdl.Duration duration) {
		return EssimDuration.of(duration.getValue(), ChronoUnit.valueOf(duration.getDurationUnit()
				.getName() + "S"));
	}
	...
	/**
	 * Converts the unit into DIDO's default units (Joule for energy, W for Power, Celcius for Temperature)
	 * 
	 * @param amount
	 *            the amount in 'unit'
	 * @param unit
